...
function addPawnMoves(board, from, color, epSquare, out) {
  const f = fileOf(from)
  const r = rankOf(from)
...
function addKnightMoves(board, from, color, out) {
  const f = fileOf(from)
  const r = rankOf(from)
  for (const [df, dr] of KNIGHT_OFFSETS) {
    const cf = f + df
...
function addSlidingMoves(board, from, color, dirs, out) {
  const f = fileOf(from)
  const r = rankOf(from)
...
function addKingMoves(board, from, color, castlingRights, out) {
  const f = fileOf(from)
  const r = rankOf(from)
  for (const [df, dr] of KING_DIRS) {
    const cf = f + df
...
function generatePseudoLegal(board, color, epSquare, castlingRights) {
  const moves = []
...
function isLegal(board, move, color, opp) {
  if (move.isCastling) {
...
export function generateLegalMoves(board, color, epSquare, castlingRights) {
